...
def send_menu(job):
    ...
    # ⛔ 30.08: «N шагов» на кнопке ОБЯЗАН считаться по той же цепочке, что реально
    # исполнится (_country_major_chain), а не плоской суммой по шагам — иначе кнопка
    # обещает 190, а по факту «Готовность» гоняется в хвосте почти сотни стран и
    # исполняется 286. Одно число — одна формула, как и сам порядок выполнения.
    total = len(_country_major_chain(steps))
...
# site.py пропускает диск) — это и есть инкрементальность, которую тракт несёт сам.
_FULL_SEQUENCE = (
    "collapse",
    "mark",
    "summarize",
...
def _country_major_chain(steps):
    """Одна страна — вся последовательность из 7 шагов подряд, потом следующая.

    ⛔ 29.08, юзер поймал: `[j for st in steps for j in st["jobs"]]` идёт ПО ШАГУ — сперва
    схлопывание всех гео, потом разметка всех. Пауза на бюджете размазывала бы
    недоделанность по всем странам разом, а не оставляла чистую границу.

    ⛔ 30.08 (два захода). Первый: цеплял «сборку»/«готовность» условно (только если у
    страны в ЭТОМ снимке уже виден `translate`) — но снимок `pipeline_state()` берётся
    ОДИН раз в начале, и для свежей страны `summarize`/`build_corpus`/`translate` там
    физически не могут появиться: файлов, по которым их считают (`tags/<гео>.json`,
    `canon.json`), ещё не существует. Итог — цикл продирался по всем странам вглубь
    ровно на то, что было видно ДО старта (обычно 2 шага из 7), и останавливался,
    требуя нового нажатия «ВСЁ ПО ПОРЯДКУ» на каждый следующий слой. Юзер: «я тебя
    спросил, если ничего не нажимать — до конца всё переведёт?» — нет, не переводило.

    Второй (юзер, дословно): не спрашивать внешнее состояние МЕЖДУ шагами страны вообще.
    Порядок фиксирован — гнать все 7 шагов подряд для одной страны, полагаясь на то, что
    каждый шаг сам решает, есть ли ему работа (пустой шаг = быстрый выход, не ошибка).
    `pipeline_state()`/`pipeline_steps()` нужны ОДИН раз — только чтобы узнать, у КАКИХ
    стран вообще есть что делать (иначе на 195 гео без единой мухи гонялись бы 195×7 job'ов
    впустую).
    """
