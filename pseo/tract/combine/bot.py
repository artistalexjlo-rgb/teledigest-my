...
def _country_major_chain(steps):
    ...
    поэтому вставлять их в хвост дёшево, не квадратичный пересчёт.

    ⛔ 30.08, юзер поймал по живому меню (95 гео на разметку, а «шагов» в кнопке 190):
    хвост цеплялся за КАЖДОЙ страной списка, а не только за той, что реально дошла до
    перевода. У массового прогона большинство стран в этом заходе доходят самое большее
    до схлопывания/разметки — сборке и готовности там смотреть ещё не на что (корпус для
    них не собран), а хвост всё равно гонял их 97 раз вхолостую. Теперь хвост идёт ТОЛЬКО
    за страной, у которой есть `translate` в этом прогоне (её данные реально новые), плюс
    ОДИН раз в самом конце — если хвост непуст, а ни одна страна перевода не завершает
    (например, «Готовность» осталась недоделанной с прошлого раза сама по себе).
    """
    ...
    if not geos:
        return list(tail)
    translate_geos = {geo for _k, geo in by_kind.get("translate") or []}
    chain = []
    tail_used = False
    for geo in geos:
        for kind in _CONTENT_KINDS:
            ...
            if pair in (by_kind.get(kind) or []):
                chain.append(pair)
        if geo in translate_geos:
            chain.extend(tail)
            tail_used = True
    if tail and not tail_used:
        chain.extend(tail)
    return chain
