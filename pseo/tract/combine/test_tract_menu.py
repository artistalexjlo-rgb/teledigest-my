...
def test_cycle_goes_country_by_country_not_step_by_step():
    """«ВСЁ ПО ПОРЯДКУ» проходит ОДНУ страну целиком (все 7 шагов подряд, фиксированным
    порядком), потом следующую — а не шаг поперёк всех стран.

    ⛔ 29.08, юзер поймал: прежняя сборка `[j for st in steps for j in st["jobs"]]` шла ПО
    ШАГУ — сперва схлопывание всех гео, потом разметка всех. Пауза на бюджете размазывала
    бы недоделанность по всем странам сразу, вместо чистой границы.

    ⛔ 30.08 (второй заход). Первая починка цепляла шаги УСЛОВНО (по снимку `pipeline_state`
    в момент нажатия) — а для свежей страны снимок физически не может видеть
    `summarize`/`build_corpus`/`translate`, файлов для них ещё нет. Итог: цикл продирался
    вглубь ровно на то, что было видно ДО старта, и останавливался. Юзер (дословно): не
    спрашивать внешнее состояние МЕЖДУ шагами страны вообще — гнать ФИКСИРОВАННЫЕ 7 шагов
    подряд, полагаясь на то, что каждый шаг сам решает, есть ли ему работа.
    """
    s = {
        "collapse": ["gr"],
        "mark": [{"geo": "gr", "n": 5}],
...
def test_every_country_in_scope_gets_the_full_fixed_sequence():
    """Страна, у которой в снимке виден только `mark` (свежая, файлов разметки ещё нет),
    всё равно получает ВСЕ 7 шагов подряд — не только те, что были видны заранее.

    ⛔ 30.08: это и есть суть починки — раньше такая страна получала бы только
    `collapse`+`mark` и застревала там до нового нажатия кнопки. Каждый шаг сам решает,
    есть ли ему работа (пустой шаг = быстрый выход) — цепочка об этом не спрашивает.
    """
    s = {
        "collapse": ["br"],
        "mark": [{"geo": "br", "n": 100}],
        "mark_n": 100,
        "summarize": [],  # br ещё не размечена — файла tags/br.json нет, summarize не видит
        "build_corpus": [],
        "to_translate": [],
        "geos": 0,
        "views": 0,
