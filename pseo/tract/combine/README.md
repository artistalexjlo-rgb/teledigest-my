...
- `/combine` — меню кнопками, шаги тракта по порядку (см. `PLAN.md`): схлопывание,
  разметка, обобщение, корпус, переводы, сборка сайта, готовность снимка, публикация.
...
- **Публикация — только `.online`.** Кнопка «8. Публикация» включается по ✅
  готовности и копирует снимок `{BRAIN}/tests/out` версией в `/root/pseo_builder/site/online`,
  переставляя симлинк `current` (nginx подхватывает без рестарта). Второго домена в
  кнопке пока нет — ждёт путь и доступ на РФ-сервере.
