...
половины показывают ОДИН путь, а без переменной — `pseo/tract/data` (28.08: дефолт переехал
из `pseo/data` вместе со всем остальным в `tract/`).
"""
...
def test_default_is_data_next_to_pseo():
    """Без переменной обе половины смотрят в `pseo/tract/data` (28.08: данные тракта
    переехали внутрь `tract/` вместе с остальным — своего каталога уровнем выше нет)."""
    got = _probe(None)
    assert got == [(PSEO / "tract" / "data").as_posix()] * 2, got
