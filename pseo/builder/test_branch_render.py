...
if __name__ == "__main__":
    ...
    # 4. ⛔ МОЛЧАЛИВОЕ УСЕЧЕНИЕ. У полок остаток резался `rest[:30]` — пункты за тридцатым
    #    исчезали без следа и без строчки в логе. Берём полку, где ветви покрывают мало, а
    #    остаток заведомо больше тридцати: ВСЁ обязано доехать до аккордеона хаба.
    big = mkpage("shelf", "Финансы и банковские услуги", 40)
    big["subshelves"] = [
        {"name": "ветка-а", "reps": ["i0", "i1", "i2", "i3"]},
        {"name": "ветка-б", "reps": ["i4", "i5", "i6", "i7"]},
    ]
    out = build([], [big])
    hubs = [k for k in out if "_s_" in k and k.count("_") == 3]
    rest_n = len(out[hubs[0]].get("faqs") or []) if hubs else 0
    good &= ok(
        rest_n == 32,  # 40 пунктов минус 8 покрытых ветвями
        "4. остаток НЕ усекается на 30 (было молчаливо)",
        "в аккордеоне %d из 32" % rest_n,
    )
