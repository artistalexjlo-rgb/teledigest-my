...
ROL = {
    ...
    "fr": {
        "цель": "objectif",
        "требование": "condition",
        "обход": "contournement",
        "обстоятельство": "contexte",
    },
    "it": {
        "цель": "obiettivo",
        "требование": "requisito",
        "обход": "alternativa",
        "обстоятельство": "contesto",
    },
    "tr": {
        "цель": "amaç",
        "требование": "koşul",
        "обход": "alternatif",
        "обстоятельство": "bağlam",
    },
    "zh": {
        "цель": "目标",
        "требование": "要求",
    ...
    "hi": {
        ...
        "требование": "शर्त",
        "обход": "विकल्प",
        "обстоятельство": "संदर्भ",
    },
    "th": {
        "цель": "เป้าหมาย",
        "требование": "เงื่อนไข",
        "обход": "ทางเลี่ยง",
        "обстоятельство": "บริบท",
    },
}
...
def run(geo, lang):
    ...
    if lang == "en":
        # en = исходник как есть, без вызова модели.
        #
        # ⛔ 08.08 я сломал это и откатил. Логика была: «у части мух `ai_lesson` по-русски,
        # переведём их на английский». Детектор считал русским исходником ЛЮБОЙ абзац,
        # где есть хоть один кириллический символ, — а таких 182 из 13 158. На деле это
        # английские абзацы с ОРИГИНАЛЬНЫМИ терминами в кавычках: «('туризъм' purpose)»,
        # «travel itinerary (маршрутный лист)», «"Госзакупки"». Термин там нужен дословно,
        # иначе человек не найдёт ни пункт меню, ни документ.
        # Хуже: `translate_texts` отбраковывает ответ с кириллицей, значит модель была
        # ВЫНУЖДЕНА термины выбросить. Итог прогона: 191 абзац переписан и обеднён,
        # ~76 обращений впустую, восстановление из БД.
        # Замер, который надо было сделать ДО прогона: текстов преимущественно
        # кириллических — НОЛЬ. Дефекта не существовало.
        text = en_text
    else:
