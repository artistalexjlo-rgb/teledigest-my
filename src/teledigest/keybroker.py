...
Все потребители (тракт и экстрактор бота — с 15.09) берут ключ ТОЛЬКО через
...
Учёт ОДИН на бота и тракт (15.09): экстрактор зовёт `call(consumer="extract", role="primary")`,
своего `gemini_quota` у него больше нет; RESERVE при общей базе — зазор для фоновых ртов,
не защита экстрактора.
"""
...
# ⭐ 15.09: база и логи мозга — в ПОДПАПКЕ brain/, а не в корне pseo_builder: бот `bots-grab`
# работает от uid 10001 и монтирует ТОЛЬКО эту подпапку (как /brain, KB_DB=/brain/keybroker.db);
# подпапка принадлежит 10001, пульт пишет туда root'ом. Логи рядом с базой (dirname(DB)).
DB = os.environ.get("KB_DB", "/root/pseo_builder/brain/keybroker.db")
...
# ⛔ СПИСОК ЗАКРЫТ (канон §0.4, 2026-08-24). Имя = ДЕЛО ВЫЗОВА, не звено: звено 4 делает три
# разных вызова, и под одним именем потом не понять, что съело квоту. Незнакомое имя
# отвергается (см. `call`) — иначе список рекомендательный и через месяц снова зоопарк.
#
# Числа щадящие, замером НЕ выверены после перехода на §0.19 (разметка без перевода отвечает
# короче, обобщение идёт двумя проходами). Уточняем, когда прогон даст факты.
#
# Старые строки `consumer_usage` под именами `facet`, `carve`, `questions`, `faq`, `synth`
# остаются как история и НЕ мигрируются — прецедент 07-19 (`consolidate` → `carve`).
CAPS = {
    "mark": 1500,  # звено 3: разметка мухи (экс-`facet`, замер: пик 482 ×3)
    "canon": 300,  # звено 4, проход А: закрытый список имён темы — 1 вызов на тему
    "assign": 300,  # звено 4, проход Б: присваивание имени из списка, пачками
    "translate": 400,  # звено 6: тексты советов (482×13яз/50 ×3)
    "labels": 200,  # звено 6: заголовки страниц
    "extract": 300,  # бот-сборщик: сообщения чата → мухи
}
...
def get_keys():
    ...
    if _KEYS is None:
        ...
        if numbered:  # numbered есть → легаси/single ИГНОРИРУЕМ (как хелпер)
            ...
            _KEYS = []
        if not _KEYS:
            # ⛔ не sys.exit: мозг живёт и в рабочем потоке бота (15.09), SystemExit там
            # не ловится как Exception и уронил бы весь процесс, а не один проход.
            raise RuntimeError("no GEMINI keys in env")
    return _KEYS
...
def call(
    ...
    """
    # ⛔ ЗАКРЫТЫЙ СПИСОК (канон §0.4). Незнакомое имя — не «новый рот с дефолтным капом», а
    # ошибка вызова: молчаливый дефолт и есть механизм, которым реестр снова зарастает.
    if consumer not in CAPS:
        raise ValueError(
            f"неизвестный потребитель {consumer!r}: список закрыт (канон §0.4), "
            f"разрешены {sorted(CAPS)}. Новое имя заводится вместе со звеном в PLAN.md"
        )
    payload = {
